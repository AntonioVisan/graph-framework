...
{
    if(m_pressedNode) m_pressedNode=nullptr;
    ...
    else if(m->button()==Qt::LeftButton)
    {
        ...
        {
            if(m_firstNode!=nullptr && m_firstNode->getIndex()!=0 && m_firstNode->getIndex()!=selected->getIndex())
            {
                std::vector<Edge> edges = m_graph.getEdges();
                bool equalEdges=false;
                for(auto &edge : edges)
                    if(edge.equals(Edge(m_firstNode,selected),m_graph.isOriented()))
                    {
                        equalEdges=true;
                        break;
                    }
